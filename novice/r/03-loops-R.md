...
The improved version of `print_words` uses a [for loop](reference#for-loop) to repeat an operation---in this case, printing---once for each thing in a collection.
...
We can name the [loop variable](reference#loop-variable) anything we like (with a few [restrictions][], e.g. the name of the variable cannot start with a digit).
